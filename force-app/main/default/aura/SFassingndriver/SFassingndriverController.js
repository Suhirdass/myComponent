({
  ...
  updateColumnSortingtsm: function (cmp, event, helper) {
     ...
     doInit : function(c, e, h) {
        h.request(c, 'assignDriverToPurchaseOrder', {recordId: c.get('v.recordId')}, function(r){
      ...
      h.navigateToRecord(c, c.get('v.recordId'));
       var toastEvent = $A.get("e.force:showToast");
        ...
        toastEvent.fire();
          ...
          /*  
            else if((c.get('v.recordId'))) {
                
              h.showToast("Success!","The record has been updated successfully.","success");
...
retrieveDriverAndRelatedRSMTSMs1 : function(c,e,h){
   h.request(c, 'retrieveDriverAndRelatedRSMTSMLight', {order: c.get('v.tsmObj')}, function(r){
  ...
  console.log(r);
       ...
       if (row.Name) {
            console.log('inside if loop');
                        row.Vehicle__c = row.Vehicle__r.Name;
                    }
           c.set("v.tsmList", rows);
  }
    */
            var rowsrsmList = r.rsmList;
 				for (var i = 0; i < rowsrsmList.length; i++) {
              console.log('inside loop');
                    var row = rowsrsmList[i];
                console.log(row);
       if (row.Name) {
...
/* retrieveDriverAndRelatedRSMTSMs : function(c,e,h){
      h.request(c, 'retrieveDriverAndRelatedRSMTSMLight', {order: c.get('v.tsmObj')}, function(r){
  console.log(r);
            c.set('v.rsmList', r.rsmList);    
           c.set('v.tsmList', r.tsmList);
