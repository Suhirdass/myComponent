({
	onInit: function (c, e, h) {
       try{
            var searchTerm = '';
            var filters = { orderByField: 'Title', isASC: true, searchTerm: searchTerm };
    ...
    onPrintSelect: function (c, e, h) {
        ...
        if(selectedFormat == 'csv'){
           h.fetchPageForFile(c);
            ...
            if(isRecent){
                currentUrl = currentUrl.replace('/s/recentdashboards','/apex/printDashboardPdf?recent=true&filters='+JSON.stringify(c.get('v.filters')));
            }else{
            	currentUrl = currentUrl.replace('/s/dashboards','/apex/printDashboardPdf?recent=false&filters='+JSON.stringify(c.get('v.filters')));
            }
