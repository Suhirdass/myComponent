({
	getIds: function(c, filters) {
        var h = this;
        const isRecent = c.get('v.isRecent');
        h.request(c, (isRecent?'getRecentDashboardIds':'getDashboardIds'), {filters: filters}, function(r){
            try{
            console.log((isRecent?'getRecentDashboardIds':'getDashboardIds'),r);
            c.set('v.ids', r.ids);
    ...
    getRecords: function(c, ids) {
        var h = this;
        h.request(c, 'getDashboards', {ids: ids, filters: c.get('v.filters')}, function(r){
            var records = r.records || [];
            console.log("getDashboards::",r);
    ...
    getRecordsForFile: function(c, ids) {
        var h = this;
        var action = c.get('c.getDashboards');
        action.setParams({
        ...
        action.setCallback(this, function(a){
            ...
            if(state == 'SUCCESS') {
                ...
                hiddenElement.download = (isRecent?'Recent Dashboards':'Dashboards')+'.csv';  // CSV file Name* you can change it.[only name not .csv] 
    ...
    convertArrayOfObjectsToCSV : function(component,objectRecords){
        ...
        var keys1 = ['Dashboard Name','Description'];
